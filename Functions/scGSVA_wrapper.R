scGSVA_wrapper <- function(seurat_dir,
                           msigDB_dir = "/rstudio-files/ccbr-data/users/Jing/CCBR_1119/Manuscript/Data/Supplementary/ccbr1119_msigDB.csv",
                           clust_ident){
  ...
  dm.formula <- as.formula(paste("~0 +", paste(c(clust_ident)), sep="+", collapse="+"))
  design=model.matrix(dm.formula, df)
  
  colnames(design) <- str_replace_all(colnames(design), clust_ident, "")
  ...
  stem_df$custom_cluster <- gsva_annot[[clust_ident]][match(stem_df$Barcode,gsva_annot$Barcode)]
